...
class ResPartner(models.Model):
    ...
    def broadcast(self, message):
        ...
        notification = {'type': 'change_detector', 'message': message}
        # self.env['bus.bus'].sendone((self._cr.dbname, 'res.partner',2), message)
        partners = self.env['res.users'].search([]).mapped('partner_id')
        for partner in partners:
            self.env['bus.bus'].sendone((self._cr.dbname, 'res.partner',partner.id), notification)
...
class Product(models.Model):
    ...
    def create(self, vals):
        ...
        self.env['res.partner'].broadcast({'p': res.id})
        return res
...
class Customer(models.Model):
    ...
    def create(self, vals):
        ...
        self.env['res.partner'].broadcast({'c': res.id})
        return res
